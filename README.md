...
- Gesture controls on the title: **left = previous, middle = play/pause, right = next**.
  Rest the pointer on a zone and a tooltip above the taskbar names it, anchored at that
  zone's left edge so it also shows where the split is. At rest the band is plain taskbar
  text — no buttons, no highlight; the About dialog shows the same split on a labeled mock
